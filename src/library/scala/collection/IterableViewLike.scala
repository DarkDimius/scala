...
{ self =>
  ...
  trait PrependedI[B >: A] extends super.PrependedT[B] with TransformedI[B] {
  ...
  protected override def newPrepended[B >: A](that: GenTraversable[B]): TransformedI[B] = new AbstractTransformedI[B] with PrependedI[B] { lazy val fst = that }
